...
public class NotePresenter {
	public static void create(final INoteView view, final INoteModel model,
			final IRightClickMenuLauncher rightClickMenuLauncher) {
		view.setContent(model.getContent());
		view.setTitle(model.getNoteName());

		final IListener modelChangedListener = new IListener() {
			@Override
			public void handleEvent() {
				String noteName = model.getNoteName();
				if (model.isDocumentDirty()) {
		...
		model.addListener(modelChangedListener, INoteModel.CONTENT_CHANGED);
		view.addDisposeListener(new IListener() {
			@Override
			public void handleEvent() {
				model.removeListener(modelChangedListener);
			}
		});
		...
		view.addListener(new IListener() {
			@Override
			public void handleEvent() {
				model.setRightClickedCooardinates(view.getLastClickedCoordinates());
				rightClickMenuLauncher.launch();
			}
		}, INoteView.RIGHT_CLICK);
	}
}
